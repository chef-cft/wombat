...
append_if_no_line "Add loopback => hostname" do
  path "/etc/hosts"
  line "127.0.0.1 chef-server.#{node['demo']['domain']} chef-server"
end
...
%w(crt key).each do |ext|
  file "/var/opt/opscode/nginx/ca/chef-server.#{node['demo']['domain']}.#{ext}" do
...
delete_lines "Remove loopback entry we added earlier" do
  path "/etc/hosts"
  pattern "^127\.0\.0\.1.*localhost.*chef-server\.#{node['demo']['domain']}.*chef-server"
end
