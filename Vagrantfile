...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "server" do |cs|
    ...
    cs.vm.hostname = "chef-server.chordata.biz"
  ...
  config.vm.define "build_node" do |bn|
    ...
    bn.vm.hostname = "build-node-1.chordata.biz"
  end

  config.vm.define "delivery" do |d|

    d.vm.network "private_network", ip: "172.31.54.11"
    d.vm.hostname = "delivery.chordata.biz"
    ...
    d.vm.provision "chef_apply" do |delivery_rb|
      delivery_rb.recipe = <<-RENDER_DELIVERY_RB
       file "/etc/delivery/delivery.rb" do
         content <<-EOH.gsub /^\s+/, ""
           delivery_fqdn "delivery.chordata.biz"
           delivery['chef_username'] = "delivery"
           delivery['chef_private_key'] = "/etc/delivery/delivery.pem"
           delivery['chef_server'] = "https://chef-server.chordata.biz/organizations/chefautomate"
    ...
    d.vm.provision "shell", inline: <<-RECONFIG_DS
      ...
      --fqdn build-node-1.chordata.biz \
      --username vagrant \
      --password vagrant \
